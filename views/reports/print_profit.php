...
// Query untuk mendapatkan data profitabilitas per produk beserta satuannya
...
class PDF extends FPDF
{
    ...
    {
        $w = array(10, 40, 25, 25, 45, 45); // Lebar kolom
        ...
        foreach ($data as $row) {
            ...
            $this->Cell($w[2], 10, $row['nama_satuan'], 1, 0, 'C'); // Tambahkan satuan produk
            $this->Cell($w[3], 10, $row['total_terjual'] . ' Botol ', 1, 0, 'C');
            $this->Cell($w[4], 10, 'Rp ' . number_format($row['total_pendapatan'], 0, ',', '.'), 1, 0, 'R');
            $this->Cell($w[5], 10, 'Rp ' . number_format($row['profit'], 0, ',', '.'), 1, 1, 'R');
        }
    }
}
...
$header = array('No', 'Nama Produk', 'Satuan', 'Total Terjual', 'Total Pendapatan (Rp)', 'Total Profit (Rp)');
